...
(require "../common/interfaces.rkt"
         ...
         racket/format
         "translate.rkt"
         (only-in "../common/json-util.rkt" Nothing))
...
(define/contract (doc-get-bindings doc decl)
  ...
  (if bindings
      ...
      empty))
;; --- 翻译缓存 (racket-lang-build) ---
(define translator-cache (box (hash)))

(define (racket-translated-language? doc)
  (define lang-info (doc-language-info doc))
  (define lang (Language-Info-language lang-info))
  (and (Known-Language? lang)
       (regexp-match? #px"^racket-cn$"
                      (symbol->string (Known-Language-name lang)))))

(define (get-translator doc)
  (define lang-info (doc-language-info doc))
  (define lang-text (Language-Prefix-text (Language-Info-prefix lang-info)))
  (define cache (unbox translator-cache))
  ;; 按文档版本缓存: 编辑后从源文本重新解析 require 模块
  (define cache-key (format "~a-~a" lang-text (Doc-version doc)))
  (hash-ref cache cache-key
            (lambda ()
              (define t
                (with-handlers ([exn:fail? (lambda (e) #f)])
                  (define suffix "")
                  (define tables-dir (find-tables-dir lang-text))
                  (if tables-dir
                      (let* ([src (send (Doc-text doc) get-text)]
                             ;; 动态加载 CN->EN 查找函数，用于识别中文 require
                             [search-map->
                              (with-handlers ([exn:fail? (lambda (e) #f)])
                                (dynamic-require
                                 (string->symbol (format "~a/search-map" lang-text))
                                 'search-map->))]
                             [modules (extract-required-modules src ""
                                                                search-map->)]
                             ;; main 默认加载 racket/main（含全部转发 deps），/base 加载 racket/base
                             [default-mod
                              (if (regexp-match? #rx"/base$" lang-text)
                                  "racket/base"
                                  "racket/main")]
                             [all-mods (remove-duplicates
                                        (cons default-mod modules))])
                        (make-scoped-translator lang-text tables-dir all-mods))
                      (load-translator lang-text))))
              (set-box! translator-cache (hash-set cache cache-key t))
              t)))
...
(define/contract (doc-completion doc pos)
  ...
  ;; 如果是翻译语言，应用翻译 (scoped 模块感知模式)
  (define translator
    (and (racket-translated-language? doc)
         (get-translator doc)))

  (define result
    (if translator
        (for/list ([completion (in-list completions)])
          (define en (symbol->string completion))
          (define cn (translate-symbol translator completion))
          (if (equal? en cn)
              (CompletionItem #:label en
                              #:insertText (Nothing)
